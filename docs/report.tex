...
\title{Practical report of ClickLabs \\
...
\begin{document}
  ...
  ClickLabs source: \url{http://clicklabs.googlecode.com}.
...
\texttt{draw-graph.sh -f dataIn.dump -f dataOut.dump \\
  ...
  \begin{figure}
      ...
      %\subfloat[\texttt{BRandomQueue}]{
      \subfigure{
      %\label{fig:test-lk-tk-count}
      \includegraphics[width=0.7\textwidth]{lk-tk-count.png}}
      \\(a) Number of packets over time
      %\subfloat[\texttt{2PRandomQueue}]{
      \subfigure{
      %\label{fig:test-lk-tk-dense}
      \includegraphics[width=0.7\textwidth]{lk-tk-dense.png}}
  ...
  \begin{figure}
      \centering
      \subfloat{
      \label{Number of packets over time}
      \includegraphics[width=0.5\textwidth]{tke-count.png}}
      \subfloat{
      \label{Distribution of packet at output}
      \includegraphics[width=0.5\textwidth]{tke-dense.png}}      
  ...
  \begin{center}
	...
	\captionof{figure}{Testing \texttt{FIFOSched} configuration: distribution of packet at output over time}
	\label{fig:test-fifo}
  \end{center}
  ...
  \begin{center}
	...
	\captionof{figure}{Testing \texttt{RRSched} configuration: distribution of packet at output over time}
	\label{fig:test-rr}
  \end{center}
  ...
  \begin{center}
	...
	\captionof{figure}{Testing \texttt{DRRSched} configuration: distribution of packet at output over time}
	\label{fig:test-drr}
  \end{center}
  ...
  \begin{center}
	...
	\captionof{figure}{Testing \texttt{WRRSched} compound element: distribution of packet at output over time}
  ...
  \begin{center}
	...
	\captionof{figure}{Testing \texttt{WRRSched} element: distribution of packet at output over time}
	\label{fig:test-wrr}
  \end{center}
  ...
  \begin{center}
	...
	\captionof{figure}{Testing \texttt{WDRRSched} compound element: distribution of packet at output over time}
  ...
  \begin{center}
	...
	\captionof{figure}{Testing \texttt{WDRRSched} element: distribution of packet at output over time}
  ...
  \begin{center}
	\includegraphics[width=0.50\textwidth]{wfq.pdf}
	\captionof{figure}{\texttt{Weighted Fair Queueing} scheduler with 3 flows}
	\label{fig:wfq}
  \end{center}
Testing scenario: three flows have the same packet rate 1 pps, and packet length 3 byte. Flow 0 and flow 1 are started at the same time, flow 2 is started 15 seconds later. Output packet rate is 1 pps. Figure \ref{fig:test-wfq} shows the output packets whose color is representation of flows. We see that flow 0 and flow 1 are not lost bandwidth when flow 2 starts. 
  \begin{center}
	\includegraphics[width=0.80\textwidth]{wfq-dense.png}
	\captionof{figure}{Testing \texttt{Weighted Fair Queueing} scheduler with 3 flows}
	\label{fig:test-wfq}
  \end{center}
...
We use the same scenario to Virtual Clock scheduler (section \ref{section:vcsched}), and result is shown in figure \ref{fig:test-vcs2}. Since Virtual Clock scheduler always assumes that all flows are active, it is not good when a flow is started long time after other flows. 
  \begin{center}
	\includegraphics[width=0.80\textwidth]{vcs2-dense.png}
	\captionof{figure}{Testing \texttt{Virtual Clock} scheduler with the same scenario of WFQ scheduler}
	\label{fig:test-vcs2}
  \end{center}
  ...
  \begin{itemize}
  	\item Element \texttt{SetTimestamp2}: it is inheritted from element \texttt{SetTimestamp} but user can use additional keyword \texttt{ADD} to define the expected deadline by changing timestamp annotation of packet. This element is also used to store deadline in payload of packet by combining it and element \texttt{StoreTimestamp}. See figure \ref{fig:setdeadline}.
        \begin{center}
	      \includegraphics[width=0.70\textwidth]{setdeadline.pdf}
	      \captionof{figure}{Element \texttt{SetTimestamp2} and \texttt{StoreTimestamp} are used to store deadline in Timestamp annotation or payload}
  ...
  After assigning deadline to packet, the EDD scheduler operates similar to a FIFO scheduler. For that reason, EDD scheduler is a "FIFO scheduler" with dropping expired packets. Figure \ref{fig:eddsched01} shows two configurations of EDD scheduler with one thing difference: in the right figure, by using element \texttt{EDDSched}, if packet misses its deadline, the scheduler will drop it and find another packets immediately (no need to run another round). If EDD scheduler is implemented by compound element (\texttt{\_edds0}), expired packet is checked in \texttt{CheckTimestamp} after the scheduler chooses it, so it needs to request the scheduler again (one more round) to choose another one. The performance of \texttt{\_edds1} should be better than \texttt{\_edds0} in the cases of many expired packets.
        ...
        \begin{center}
	      \includegraphics[width=0.8\textwidth]{eddsched01.pdf}
	      \captionof{figure}{Two configurations of EDD Scheduler with basic function}
        ...
        \begin{figure}
          \centering
          \subfloat[Number of packets over time]{
            \includegraphics[width=0.5\textwidth]{edds11-count.png}
	          \label{fig:test-edd-ideal-count}}
          \subfloat[Distribution of packets of each flow at output over time]{
            \includegraphics[width=0.5\textwidth]{edds11.png}
	          \label{fig:test-edd-ideal-dense}}	          
        ...
        \begin{figure}
          \subfloat[Number of packets over time]{
          \centering
            \includegraphics[width=0.5\textwidth]{edds1-count.png}
	          \label{fig:test-edd1-count}}
          \subfloat[Distribution of packets of each flow at output over time]{
            \includegraphics[width=0.5\textwidth]{edds1.png}
	          \label{fig:test-edd1-dense}}	          
  ...
  \begin{center}
	  \includegraphics[width=0.9\textwidth]{edds2.pdf}
	  \captionof{figure}{EDD Scheduler with improvement \texttt{\_EDDSched2}}
	  \label{fig:edds2}
    \end{center}
  ...
  Figure \ref{fig:edds2} shows the Click configuration of improved EDD scheduler with this dropping policy that does not always allow packets stored in queue (\texttt{DeadlineQueue}). Element \texttt{ShouldDrop} makes the decision of dropping packets: packet is dropped if \begin{align*}\alpha * OUTPUT\_RATE * DEADLINE < GlobalQueue.length + DeadlineQueue.length\end{align*}
    ...
    \begin{figure}
      \centering
      \subfloat[Number of packets over time]{
        \includegraphics[width=0.5\textwidth]{edds2-count.png}
        \label{fig:test-edd2-count}}
      \subfloat[Distribution of packets of each flow at output over time]{
        \includegraphics[width=0.5\textwidth]{edds2.png}
        \label{fig:test-edd2-dense}}	          
  ...
  \begin{center}
	  \includegraphics[width=0.5\textwidth]{edds3.pdf}
	  \captionof{figure}{EDD Scheduler with improvement \texttt{\_EDDSched3}}
	  \label{fig:edds3}
    \end{center}
  ...
  Figure \ref{fig:edds3} is the configuration using this \texttt{ShiftQueue}. We also use the same second scenario in the section \ref{section:eddbasic} to test this configuration. In figure \ref{fig:test-edd3}, we see that the fairness is comparable to the case of large size queue. 
    \begin{figure}
      \centering
      \subfloat[Number of packets over time]{
        \includegraphics[width=0.5\textwidth]{edds3-count.png}
        \label{fig:test-edd3-count}}
      \subfloat[Distribution of packets of each flow at output over time]{
        \includegraphics[width=0.5\textwidth]{edds3.png}
        \label{fig:test-edd3-dense}}
  ...
  \begin{center}
	...
	\captionof{figure}{Testing \texttt{WRED2}}
  ...
  \lstinputlisting[style=iizke]{init.sh.man}

  \subsection{Command \texttt{visual-clicky.sh}}
  \lstinputlisting[style=iizke]{visual-clicky.sh.man}

  \subsection{Command \texttt{update-elements.sh}}
  \lstinputlisting[style=iizke]{update-elements.sh.man}
  
  \subsection{Command \texttt{eclick-compile.sh}}
  \lstinputlisting[style=iizke]{eclick-compile.sh.man}

  \subsection{Command \texttt{convert-click-dump.sh}}
  \lstinputlisting[style=iizke]{convert-click-dump.sh.man}
  
  \subsection{Command \texttt{draw-graph.sh}}
  \lstinputlisting[style=iizke]{draw-graph.sh.man}
  
  \subsection{Command \texttt{draw-graph-negotiation.sh}}
  \lstinputlisting[style=iizke]{draw-graph-negotiation.sh.man}
