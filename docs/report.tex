...
\texttt{draw-graph.sh -f dataIn.dump -f dataOut.dump \\
  ...
  \begin{itemize}
  	...
  	\item \texttt{RatedNegotiablePolicer2}: input packets are gone through \texttt{RatedLeakyBucketShaper} (with leaky rate $R_L = \frac{CIR * (CBS + EBS)}{CBS}$) and then gone through \texttt{RatedTokenBucketShaper2} (with token rate $R_L = CIR$ and burst duration is $EBS$). At output, we gurantee that in interval time $T = \frac{CBS}{CIR}$, maximum number of output packet is $(CBS + EBS)$ and flow is shapped to rate $CIR$ with a maximum burst duration $EBS$. 
  \end{itemize}
  \begin{center}
  ...
  \end{center}
  Figure \ref{fig:negotiation-check} shows a testing result with \texttt{RatedNegotiablePolicer4}. Source is a \texttt{ProbUncontrolledFlow} with maximum rate 500 pps. This flow goes through \texttt{RatedNegotiablePolicer4} with CBS = 50, EBS = 10, CIR = 100 (pps). As shown in figure \ref{fig:negotiation-check}, all packet in a window time $T = 0.5s$, the number of output packets is trimmed at 60 packets follow rules of negotiation. 
    \begin{figure}
    \centering
