#ifndef CLICK_RANDOM_INFINITESOURCE_HH
#define CLICK_RANDOM_INFINITESOURCE_HH
...
RandomInfiniteSource([DATA, LIMIT, BURST, ACTIVE, I<KEYWORDS>])
...
class RandomBytePacket;

class RandomInfiniteSource : public Element, public ActiveNotifier { public:
//class RandomInfiniteSource : public InfiniteSource, public ActiveNotifier { public:

  RandomInfiniteSource();
  ~RandomInfiniteSource();

  const char *class_name() const		{ return "RandomInfiniteSource"; }
...
class RandomBytePacket { public:
  RandomBytePacket ();
  ~RandomBytePacket ();

  Packet *clone(uint32_t position);
  void kill();
  void make(const void *data, uint32_t length);
