#ifndef CLICK_RANDINFINITESOURCE_HH
#define CLICK_RANDINFINITESOURCE_HH
...
RandInfiniteSource listens for downstream full notification.

=h count read-only
Returns the total number of packets that have been generated.
=h reset write-only
Resets the number of generated packets to 0. The RandInfiniteSource will then
...
class RandInfiniteSource : public Element, public ActiveNotifier { public:

  RandInfiniteSource();
  ~RandInfiniteSource();

  const char *class_name() const		{ return "RandInfiniteSource"; }
  ...
  bool run_task(Task *);
  Packet *pull(int);
  Packet *rebuild_packet(int32_t byteid);
 protected:
