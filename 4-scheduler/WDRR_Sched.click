// WDRR_Sched.click
// Simulate WDRR scheduler with at most 10 inputs (flows)

elementclass WDRRSched {
  ...
  input[2] -> rs2::RoundRobinSwitch;
  input[3] -> rs3::RoundRobinSwitch;
  input[4] -> rs4::RoundRobinSwitch;
  input[5] -> rs5::RoundRobinSwitch;
  input[6] -> rs6::RoundRobinSwitch;
  input[7] -> rs7::RoundRobinSwitch;
  input[8] -> rs8::RoundRobinSwitch;
...
// Initialize flows
s0::RatedSource(LENGTH 500, RATE 100, ACTIVE true);
s1::RatedSource(LENGTH 1000, RATE 200, ACTIVE true);
s2::RatedSource(LENGTH 1500, RATE 300, ACTIVE true);
s3::RatedSource(LENGTH 1500, RATE 400, ACTIVE false);
s4::RatedSource(LENGTH 500, RATE 500, ACTIVE false);
s5::RatedSource(LENGTH 1500, RATE 600, ACTIVE false);
s6::RatedSource(LENGTH 700, RATE 700, ACTIVE false);
s7::RatedSource(LENGTH 800, RATE 800, ACTIVE false);
s8::RatedSource(LENGTH 900, RATE 900, ACTIVE false);
s9::RatedSource(LENGTH 1000, RATE 1000, ACTIVE false);
...
ps[0] -> ToDump(output-w1-l500, SNAPLEN 1) -> c0::Counter -> Discard;
ps[1] -> ToDump(output-w2-l1000, SNAPLEN 1) -> c1::Counter -> Discard;
ps[2] -> ToDump(output-w3-l1500, SNAPLEN 1) -> c2::Counter -> Discard;
